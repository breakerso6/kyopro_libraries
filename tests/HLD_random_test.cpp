...
void check_tree(const vector<vector<ll>>& g, int root) {
    int n = (int)g.size();
    HLD hld(g, root);
    ...
    for (int v = 0; v < n; v++) {
        for (int d = 0; d <= n + 1; d++) {
            int expected = naive.level_ancestor(v, d);
            int actual = hld.level_ancestor(v, d);
...
void check_all_roots(const vector<vector<ll>>& g) {
    for (int root = 0; root < (int)g.size(); root++) check_tree(g, root);
}
